...
FrameworkApp.prototype.on = function() {
  return this.express.on.apply(this.express, Array.prototype.slice.call(arguments));
}
...
/**
   ...
   * It initializes a router and adds it to 'mainRouter'.
   */
FrameworkApp.prototype.lazyrouter = function() {
  var lazyrouter = Routing();
  this.addRouters(lazyrouter);
  return lazyrouter;
}

methods.concat(['all', 'use']).forEach(function(method){
  FrameworkApp.prototype[method] = function(path){
    ...
    if (method === 'use' && arguments.length === 1) {
      this.addRouters.apply(this, Array.prototype.slice.call(arguments));
    } else {
      var r = this.lazyrouter();
      r[method].apply(r, Array.prototype.slice.call(arguments));
    }
    return this;
  };
});
